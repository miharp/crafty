...
It will be put in the `./server-data/ssl` directory, or any other volume you may have mounted for `/etc/puppetlabs/puppet/ssl`.
...
One can then mount the `./server-data/ssl` directory or whatever mount one have to the additional container, which shall use the certs.
